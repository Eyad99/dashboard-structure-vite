...
export function SidebarLinks(props: { routes: RoutesType[]; [x: string]: any }) {
	...
	const createLinks = (routes: RoutesType[]) => {
		return routes.map((route, key) => {
			...
			// if (route.collapse) {
			// 	return (
			// 		<Accordion defaultIndex={activeRoute(route.path.toLowerCase()) ? 0 : 1} allowToggle key={key}>
			...
			//               ${mini === false ? 'block' : mini === true && hovered === true ? 'block' : 'block xl:hidden'}`}
			// 								display={mini === false ? 'block' : mini === true && hovered === true ? 'block' : { base: 'block', xl: 'none' }}
				...
				return (
					<Link to={route.layout + route.path} key={key}>
						{route.icon ? (
							...
							>
								<li className='my-[3px] flex cursor-pointer items-center px-[22px]' key={key}>
									<span
